...
function Card({
  ...
  return (
    <div
      ...
      className={cn(
        "group/card bg-card text-card-foreground ring-border flex flex-col gap-(--card-spacing) overflow-hidden rounded-2xl py-(--card-spacing) text-sm shadow-lg shadow-black/[0.03] ring-1 transition-all duration-200 ease-out [--card-spacing:--spacing(6)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[interactive]:cursor-pointer data-[interactive]:hover:-translate-y-0.5 data-[interactive]:hover:shadow-xl data-[size=sm]:[--card-spacing:--spacing(4)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 dark:shadow-black/20 *:[img:first-child]:rounded-t-2xl *:[img:last-child]:rounded-b-2xl",
        className
      )}
      {...props}
    />
